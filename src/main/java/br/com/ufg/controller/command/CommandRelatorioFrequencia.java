...
public class CommandRelatorioFrequencia implements Command {
	...
	public CommandRelatorioFrequencia(ServletContext context, InterfacePool pool, DisciplinaDAO daoDisciplina, TurmaDAO daoTurma, DataDAO daoData, AlunoDAO daoAluno){
		...
		try {
			if(daoDisciplina.pertenceProfessor(disciplina.getCodigo(), professor) && daoTurma.pertenceDisciplina(turma.getCodigo(), disciplina.getCodigo())){
				...
				try{
		            ...
		            JasperReport relatorioJasper = (JasperReport)JRLoader.loadObjectFromFile(context.getRealPath("/WEB-INF/reports/Frequencia.jasper"));
